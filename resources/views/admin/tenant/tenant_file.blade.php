        ...
        <div class="col-lg-12">
            <div class="card" id="header1">
                <div class="card-header align-items-center d-flex table-main-heading" id="card-header">
                    <h4 class="card-title mb-0 flex-grow-1" id="h1">{{ isset($filesedit) ? 'Update Files' : 'Create Files' }}</h4>
                </div><!-- end card header -->
                <div class="card-body">
                    <div class="live-preview">
                        <form
                            action="{{ isset($filesedit) ? route('admin.tenant-files.update', $filesedit->id) : route('admin.tenant-files.store') }}"
                            ...
                            <div class="card-body field_wrapper4">
                                <div class="row clone4">
                                    <div class="col-xxl-3 col-md-3">
                                        ...
                                        <select class="select2 form-select js-example-basic-single" id="tenant_code"
                                            name="tenant_code[]">
                                            @if (isset($city))
                                                <option value="{{ $city->tenant_code }}" selected>{{ $city->tenant_code }}
                                                </option>
                                            @else
                                    ...
                                    <div class="col-sm-1">
                                        <br />
                                        <a href="javascript:void(0);" id="btn-btn" class="add_button4 btn btn-success"
                                            title="Add field">+</a>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="row mt-2">
                        <div class="col-xxl-3 col-md-6">
                            <div class="input-group">
                                <button id="btn-btn" class="btn btn-primary"
                                    type="submit">{{ isset($filesedit) ? 'Update' : 'Submit' }}</button>
                            </div>
                        </div>
                    </div>
                    </form>
                        ...
                        <tbody>
                            @foreach ($files as $f)
                                <tr>
                                    ...
                                    <td>{{ $f->file_name ?? '' }}</td>
                                    <td><a href="{{ asset('upload/tenant/files') . '/' . $f->attachment }}"
                                            target="_ blank">{{ $f->attachment ?? '' }}</a></td>
                                    <td>{{ $f->created_at ?? '' }}</td>
                                    <td>
                                        <div class="dropdown">
                                            ...
                                            <ul class="dropdown-menu" aria-labelledby="dropdownMenuLink">
                                                ...
                                                <form id="delete-form-{{ $bid }}"
                                                    action="{{ route('admin.tenant-files.destroy', $bid) }}" method="post"
                                                    style="display: none;">
                                                    @method('DELETE')
                                                    @csrf
                                                </form>
...
@section('script-area')
    ...
    <select class="form-control select2 form-select" name="tenant_code[]">' + '{!! $filehtml !!}\
       </select></div>\
            <div class="col-sm-4">\
