                    ...
                    <table id="example" class="display table table-bordered dt-responsive dataTable dtr-inline" style="width: 100%;" aria-describedby="ajax-datatables_info">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ $u->buildingDetails->name??$u->unitRef->buildingDetails->name??'' }}</td>
                                <td>{{ $u->buildingDetails->building_type??$u->unitRef->buildingDetails->building_type??'' }}</td>
                                <td>{{ $u->unitTypeDetails->name ??$u->unitRef->unitTypeDetails->name??'' }}</td>
                                <td>{{ $u->unit_no ??$u->unitRef->unit_no??'' }}</td>
                                <td>{{ $u->unitfloor->name??$u->unitRef->unitfloor->name??'' }}</td>
                                <td>{{ $u->unit_size??$u->unitRef->unit_size??''}}</td>
                                <td>{{ $u->actual_rent??$u->unitRef->actual_rent??''}}</td>
                                @if($type=='vacant')
                                <td>{{isset($u->vacant_date)?carbon\carbon::parse($u->vacant_date)->format('d-M-Y'):''}}</td>
                                <td>{{ $u->reason_vacant??''}}</td>
                                @endif
                                @if($type=='legal process')
                                <td>{{carbon\carbon::parse($u->updated_at)->format('d-M-Y')??carbon\carbon::parse($u->created_at)->format('d-M-Y')}}</td>
                                <td>{{$u->status??''}}</td>
                                <td>{{$u->remark??''}}</td>
                                @endif
                                @if($type=='all')
